...
CUR_UID=$(id -u prowlarr)
CUR_GID=$(id -g prowlarr)

if [ "$PGID" != "$CUR_GID" ]; then
  sed -i "s/^prowlarr:x:[0-9]*:/prowlarr:x:${PGID}:/" /etc/group
fi
if [ "$PUID" != "$CUR_UID" ]; then
  sed -i "s/^prowlarr:x:[0-9]*:[0-9]*:/prowlarr:x:${PUID}:${PGID}:/" /etc/passwd
fi

for dir in /config /run/prowlarr-temp; do
  if [ -d "$dir" ]; then
